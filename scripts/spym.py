...
class spym_pulse(pynlo.light.PulseBase.Pulse):
    ...
    def plot(self,ax=[],cutoff_percent=0.0,phase=0,linestyles=['b-','r--'],normalize=1,labels=['Temporal','Spectrum','Phase'],x_axis_wave=False, log_scale=False, temporal_plot = True):
        '''
        Plotting pulses
        '''
        ...
        if x_axis_wave == 1:
            ...
            ax[1].set_ylim(cutoff_percent*np.max((spectrum*freq**2)),1.05*np.max((spectrum*freq**2)))
            if log_scale == True:
                ax[1].set_yscale("log")
            ...
            if log_scale == True:
                ax[1].set_yscale("log")
            ax[1].set_ylim([10**(-5),1.05*np.max(spectrum)])
        ...
        if phase == 1:
            ...
            if x_axis_wave == 1:
                ax[0].plot((299700.0/freq)[::-1],spectrum*0-10.0,linestyleP,label=labels[2])
                ax[2].plot((299700.0/freq)[::-1],phase[::-1],linestyleP,label=labels[2])
                ax[1].set_xlim([299700.0/(freq[cropIndex_spec][-1]),299700.0/(freq[cropIndex_spec][0])])
